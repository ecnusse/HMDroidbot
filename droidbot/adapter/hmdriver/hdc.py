...
from .execption import HdcError, DeviceNotFoundError
...
class HdcWrapper:
    ...
    def forward_port(self, rport: int) -> int:
        ...
        result = _execute_command(
            f"{self.hdc_prefix} -t {self.serial} fport tcp:{lport} tcp:{rport}"
        )
        if result.exit_code != 0:
    ...
    def send_file(self, lpath: str, rpath: str):
        result = _execute_command(
            f"{self.hdc_prefix} -t {self.serial} file send {lpath} {rpath}"
        )
        if result.exit_code != 0:
    ...
    def recv_file(self, rpath: str, lpath: str):
        result = _execute_command(
            f"{self.hdc_prefix} -t {self.serial} file recv {rpath} {lpath}"
        )
        if result.exit_code != 0:
    ...
    def uninstall(self, bundlename: str):
        result = _execute_command(
            f"{self.hdc_prefix} -t {self.serial} uninstall {bundlename}"
        )
        if result.exit_code != 0:
    ...
    def install(self, apkpath: str):
        ...
        result = _execute_command(
            f"{self.hdc_prefix} -t {self.serial} install {quoted_path}"
        )
        if result.exit_code != 0:
    ...
    def current_app(self) -> Tuple[str, str]:
        ...
        def __extract_info(output: str):
            ...
            mission_blocks = re.findall(
                r"Mission ID #[\s\S]*?isKeepAlive: false\s*}", output
            )

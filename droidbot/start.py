...
from .utils import get_yml_config, identify_device_serial, load_yml_args, check_package
import logging
...
def parse_args():
    ...
    load_yml_args(options, config_dict)
    identify_device_serial(options)
    check_package(opts=options)
...
def main():
    ...
    if not opts.output_dir and opts.cv_mode:
        logger.error(
            "To run in CV mode, you need to specify an output dir (using -o option)."
        )
